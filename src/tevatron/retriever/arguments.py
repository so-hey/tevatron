...
class ModelArguments:
    ...
    pooling: str = field(
        default="cls", metadata={"help": "pooling method for query and passage encoder"}
    )
    normalize: bool = field(
        default=False, metadata={"help": "normalize query and passage representations"}
    )

    temperature: float = field(
        default=1.0, metadata={"help": "temperature for softmax"}
    )
...
class DataArguments:
    ...
    dataset_config: str = field(
        default=None,
        metadata={
            "help": "huggingface dataset config, useful for datasets with sub-datasets"
        },
    )
    ...
    corpus_config: str = field(
        default=None,
        metadata={
            "help": "huggingface dataset config for corpus, useful for datasets with sub-datasets"
        },
    )
    ...
    corpus_split: str = field(default="train", metadata={"help": "corpus split"})

    train_yaml: str = field(
        default=None,
        metadata={
            "help": "yaml file for training datasets, if there is more multiple datasets used for training"
        },
    )
    ...
    dataset_shard_index: int = field(
        default=0,
        metadata={
            "help": "shard index to use, to be used with dataset_number_of_shards"
        },
    )
    ...
    positive_passage_no_shuffle: bool = field(
        default=False,
        metadata={"help": "always use the first positive passage for training"},
    )
    negative_passage_no_shuffle: bool = field(
        default=False,
        metadata={"help": "always use the first n negative passages for training"},
    )
    ...
    num_proc: int = field(
        default=1,
        metadata={"help": "number of processes to use for loading the dataset"},
    )
